...
export default class NewPage {
  ...
  #setupForm() {
    ...
    this.#form.addEventListener('submit', async (event) => {
      ...
      const damageLevel = this.#form.elements.namedItem('damageLevel').value;
      if (!['severe', 'moderate', 'minor'].includes(damageLevel)) {
        alert('Pilih tingkat kerusakan terlebih dahulu!');
        return;
      }
  ...
  async initialMap() {
    // Inisialisasi map di elemen #map
    this.#map = await Map.build('#map', {
      zoom: 15,
      locate: true,
    });

    // Ambil input latitude & longitude
    ...
    this.#marker = this.#map.addMarker([initialLat, initialLng], {}, { content: 'Lokasi laporan' });

    // Update marker & input saat map diklik
    this.#map.#map.on('click', (e) => {
      ...
      this.#marker = this.#map.addMarker([lat, lng], {}, { content: 'Lokasi laporan' });
    });
  }
